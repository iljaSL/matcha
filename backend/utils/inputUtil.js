...
export default {
  lastname: (data) => {
    ...
    if (data.length < 2 || data.length > 20)
      return { error: "lastname needs to be between 2 and 19 chars long" };
  ...
  firstname: (data) => {
    ...
    if (data == null || data == "") return { error: "Input can not be empty!" };
    if (/\s/.test(data))
      return { error: "firstname can not contain empty spaces!" };
    if (!data.match(regex)) return { error: "invalid firstname" };
    if (data.length < 2 || data.length > 20)
      return { error: "firstname needs to be between 2 and 19 chars long" };
    else return { status: "valid" };
  },
  ...
  username: async (data) => {
    ...
    if (data == null || data == "") return { error: "Input can not be empty!" };
    if (/\s/.test(data))
      return { error: "username can not contain empty spaces!" };
